...
class duration(
    ...
    __Lexical_re = re.compile(
        "^(?P<neg>-?)P((?P<years>\d+)Y)?((?P<months>\d+)M)?((?P<days>\d+)D)?(?P<Time>T((?P<hours>\d+)H)?((?P<minutes>\d+)M)?(((?P<seconds>\d+)(?P<fracsec>\.\d+)?)S)?)?$"
    )
    ...
    def __new__(cls, *args, **kw):
        ...
        if kw.get("_nil"):
            ...
            if isinstance(value, six.string_types):
                ...
                if (
                    len(args) == len(cls._ValidFields) + 1
                    or len(args) == len(cls._ValidFields) + 5
                ):
