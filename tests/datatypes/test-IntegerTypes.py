...
class _TestIntegerType(object):
    ...
    def testRange(self):
        if self.MIN_IN_RANGE is not None:
            if not ((self.MIN_IN_RANGE - 1) in self.PARENT_EXCLUDE):
                self.assertRaises(
                    SimpleTypeValueError, self.THIS_TYPE, self.MIN_IN_RANGE - 1
                )
        ...
        if self.MAX_IN_RANGE is not None:
            ...
            if not ((self.MAX_IN_RANGE + 1) in self.PARENT_EXCLUDE):
                self.assertRaises(
                    SimpleTypeValueError, self.THIS_TYPE, self.MAX_IN_RANGE + 1
                )
    ...
    def testStringConversion(self):
        ...
        for n in numbers:
            ...
            if ((self.MIN_IN_RANGE is None) or (self.MIN_IN_RANGE <= n)) and (
                (self.MAX_IN_RANGE is None) or (n <= self.MAX_IN_RANGE)
            ):
                bs = self.THIS_TYPE(s)
