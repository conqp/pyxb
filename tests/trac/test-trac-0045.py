...
def CreateDocumentationNode(content):
    xmls = (
        '<xs:annotation xmlns:xs="%s"><xs:documentation>%s</xs:documentation></xs:annotation>'
        % (pyxb.namespace.XMLSchema.uri(), content)
    )
...
class TestTrac_0045(unittest.TestCase):
    def testSimple(self):
        self.assertEqual(
            "hi there!", CreateDocumentationNode("hi there!").asDocString()
        )
        self.assertEqual(
            ' "hi there!" ', CreateDocumentationNode('"hi there!"').asDocString()
        )
        self.assertEqual(
            "''' docstring! '''",
            CreateDocumentationNode('""" docstring! """').asDocString(),
        )
        self.assertEqual(
            "inner ''' docstring!",
            CreateDocumentationNode('inner """ docstring!').asDocString(),
        )
