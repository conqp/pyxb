...
class TestTrac0203(unittest.TestCase):
    def testBasic(self):
        ...
        if sys.version_info[:2] < (2, 7):
            self.assertRaises(
                pyxb.UnboundElementError, unbound.toxml, "utf-8", root_only=True
            )
        else:
            ...
            self.assertEqual(
                "Instance of type XsdWithHyphens has no bound element for start tag",
                str(e),
            )
    ...
    def testOverride(self):
        ...
        self.assertEqual(
            xmld, unbound.toxml("utf-8", root_only=True, element_name="root")
        )
