...
class TestIssue13(unittest.TestCase):
    def testEnumerations(self):
        instance = CreateFromDocument(
            six.u('<qne xmlns:ns1="urn:issue13.1">ns1:one</qne>')
        )
        self.assertEqual(instance.value(), ns1.createExpandedName("one"))
        with self.assertRaises(pyxb.SimpleFacetValueError) as cm:
            instance = CreateFromDocument(
                six.u('<qne xmlns:ns1="urn:issue13.1">ns1:un</qne>')
            )
        self.assertEqual(
            "Type tQNE enumeration constraint violated by value {urn:issue13.1}un",
            str(cm.exception),
        )
        with self.assertRaises(pyxb.SimpleFacetValueError) as cm:
            instance = CreateFromDocument(
                six.u('<qne xmlns:ns1="urn:issue13.2">ns1:one</qne>')
            )
        self.assertEqual(
            "Type tQNE enumeration constraint violated by value {urn:issue13.2}one",
            str(cm.exception),
        )
    ...
    def testElements(self):
        ...
        self.assertEqual(
            six.u(
                '<qn xmlns:xml="http://www.w3.org/XML/1998/namespace">xml:nons</qn>'
            ).encode("utf-8"),
            xmld,
        )
