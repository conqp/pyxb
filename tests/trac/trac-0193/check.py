...
assert (
    trac193.Money._UseForTag("Currency").elementBinding().typeDefinition()
    == xs.anySimpleType
)
assert (
    trac193.Money._UseForTag("Season").elementBinding().typeDefinition() == xs.anyType
)

rdv = trac193.OpenDeliveries._UseForTag("Value").elementBinding().typeDefinition()
assert rdv == trac193.Value
assert (
    trac193.OpenDeliveries._UseForTag("Currency").elementBinding().typeDefinition()
    == trac193.Currency
)
assert (
    trac193.OpenDeliveries._UseForTag("Season").elementBinding().typeDefinition()
    == trac193.Season
)
