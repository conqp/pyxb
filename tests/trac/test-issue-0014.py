...
class TestIssue14(unittest.TestCase):
    def testAddAttributes(self):
        x = wildcard()
        self.assertEqual(
            six.u("<wildcard/>"), x.toxml("utf-8", root_only=True).decode("utf-8")
        )
        x.attr = False
        self.assertEqual(
    ...
    def testAddElements(self):
        ...
        x = wildcard()
        self.assertEqual(
            six.u("<wildcard/>").encode("utf-8"), x.toxml("utf-8", root_only=True)
        )
        x._appendWildcardElement(elt)
        self.assertEqual(
            six.u(
                '<wildcard xmlns:n2="urn:issue14.2"><n2:e2>content</n2:e2></wildcard>'
            ).encode("utf-8"),
            x.toxml("utf-8", root_only=True),
        )
