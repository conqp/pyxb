...
class TestTrac_0060(unittest.TestCase):
    ...
    ConflictString = (
        '<integer xsi:type="String" xmlns:xsi="%s"><child>%s</child></integer>'
        % (XSI.uri(), NotAnInteger)
    )
    ConflictStringInteger = (
        '<integer xsi:type="String" xmlns:xsi="%s"><child>%d</child></integer>'
        % (XSI.uri(), AnInteger)
    )
    UntypedIntegerElement = "<element><child>%d</child></element>" % (AnInteger,)
    TypedElement = (
        '<element xsi:type="Integer" xmlns:xsi="%s"><child>%d</child></element>'
        % (XSI.uri(), AnInteger)
    )
    # There is no type Float in the schema
    BogusTypedElement = (
        '<element xsi:type="Float" xmlns:xsi="%s"><child>%d</child></element>'
        % (XSI.uri(), AnInteger)
    )
    BogusTypedInteger = (
        '<integer xsi:type="Float" xmlns:xsi="%s"><child>%d</child></integer>'
        % (XSI.uri(), AnInteger)
    )
