...
schema_path = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "../schemas/test-mg-all.xsd")
)
...
class TestMGAll(unittest.TestCase):
    def testRequired(self):
        ...
        self.assertRaises(
            IncompleteElementContentError, required.createFromDOM, dom.documentElement
        )
        ...
        self.assertTrue(
            isinstance(instance.first, required.memberElement("first").typeDefinition())
        )
        self.assertTrue(
            isinstance(
                instance.second, required.memberElement("second").typeDefinition()
            )
        )
        self.assertTrue(
            isinstance(instance.third, required.memberElement("third").typeDefinition())
        )
    ...
    def testRequiredTooMany(self):
        ...
        dom = pyxb.utils.domutils.StringToDOM(xmlt)
        self.assertRaises(
            UnrecognizedContentError, required.createFromDOM, dom.documentElement
        )
    ...
    def testThirdOptional(self):
        ...
        self.assertTrue(
            isinstance(
                instance.first,
                thirdOptional._ElementMap["first"].elementBinding().typeDefinition(),
            )
        )
        self.assertTrue(
            isinstance(
                instance.second,
                thirdOptional._ElementMap["second"].elementBinding().typeDefinition(),
            )
        )
        ...
        self.assertTrue(
            isinstance(
                instance.third,
                thirdOptional._ElementMap["third"].elementBinding().typeDefinition(),
            )
        )
    ...
    def testOptional(self):
        ...
        instance = optional.createFromDOM(dom.documentElement)
        self.assertTrue(
            isinstance(instance.first, optional.memberElement("first").typeDefinition())
        )
        self.assertTrue(
            isinstance(
                instance.second, optional.memberElement("second").typeDefinition()
            )
        )
        self.assertTrue(
            isinstance(instance.third, optional.memberElement("third").typeDefinition())
        )

        xmlt = (
            '<ns1:optional xmlns:ns1="URN:test-mg-all"><first/><third/></ns1:optional>'
        )
        dom = pyxb.utils.domutils.StringToDOM(xmlt)
        instance = optional.createFromDOM(dom.documentElement)
        self.assertTrue(
    ...
    def testOptionalTooMany(self):
        ...
        dom = pyxb.utils.domutils.StringToDOM(xmlt)
        self.assertRaises(
            UnrecognizedContentError, optional.createFromDOM, dom.documentElement
        )
    ...
    def testMany(self):
        for body in ["abcdefgh", "fghbcd", "bfgcahd"]:
            xmlt = '<ns1:many xmlns:ns1="URN:test-mg-all">%s</ns1:many>' % (
                "".join(["<%s/>" % (_x,) for _x in body]),
            )
            dom = pyxb.utils.domutils.StringToDOM(xmlt)
            ...
            self.assertEqual(
                '<ns1:many xmlns:ns1="URN:test-mg-all">%s</ns1:many>'
                % ("".join(len(body) * ["X"]),),
                rev,
            )
        ...
        if sys.version_info[:2] >= (2, 7):
            ...
            self.assertEqual(
                many.memberElement("b"),
                acceptable[0].elementDeclaration().elementBinding(),
            )
