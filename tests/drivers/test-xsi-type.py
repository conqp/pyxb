...
schema_path = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "../schemas/xsi-type.xsd")
)
...
class TestXSIType(unittest.TestCase):
    def testFailsNoType(self):
        ...
        self.assertRaises(
            pyxb.AbstractInstantiationError, CreateFromDOM, doc.documentElement
        )
    ...
    def testSubstitutions(self):
        xmlt = six.u(
            '<elt attrOne="low" xsi:type="alt1" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"><first>content</first></elt>'
        )
        doc = pyxb.utils.domutils.StringToDOM(xmlt)
        instance = CreateFromDOM(doc.documentElement)
        self.assertEqual("content", instance.first)
        self.assertEqual("low", instance.attrOne)
        xmlt = six.u(
            '<elt attrTwo="hi" xsi:type="alt2" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"><second/></elt>'
        )
        doc = pyxb.utils.domutils.StringToDOM(xmlt)
        instance = CreateFromDOM(doc.documentElement)
    ...
    def testMultilevel(self):
        ...
        xmlt = six.u(
            '<oneFloor xsi:type="restaurant" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"><basement>concrete</basement><lobby>tiled</lobby><room>eats</room></oneFloor>'
        )
        doc = pyxb.utils.domutils.StringToDOM(xmlt)
        instance = CreateFromDOM(doc.documentElement)
    ...
    def testConstructor(self):
        kw = {"basement": "concrete", "lobby": "tiled", "room": "eats"}
        ctd = restaurant_(**kw)
        dom = ctd.toDOM(element_name="restaurant").documentElement
        xmlt = six.u(
            "<restaurant><basement>concrete</basement><lobby>tiled</lobby><room>eats</room></restaurant>"
        )
    ...
    def testNesting(self):
        instance = block(
            oneFloor=[
                restaurant(basement="dirt", lobby="tile", room="messy"),
