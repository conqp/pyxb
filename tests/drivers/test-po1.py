...
schema_path = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "../schemas/po1.xsd")
)
...
Anytown, AS  12345-6789"""
    ...
    def testPurchaseOrder(self):
        ...
        self.assertTrue(
            (not isinstance(loc, pyxb.utils.utility.Locatable_mixin))
            or (58 == loc.columnNumber)
        )
        loc = po2.billTo.name._location()
        self.assertTrue(
            (not isinstance(loc, pyxb.utils.utility.Locatable_mixin))
            or (131 == loc.columnNumber)
        )
