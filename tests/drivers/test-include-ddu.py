...
schema_path = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "../schemas/test-include-ddu.xsd")
)
...
class TestIncludeDD(unittest.TestCase):
    ...
    def testDefault(self):
        xmls = '<entry xmlns="%s"><from>one</from><to>single</to></entry>' % (
            Namespace.uri(),
        )
        # Default namespace applies to from which should be in no namespace
        self.__basis_log.setLevel(logging.ERROR)
        self.assertRaises(
            pyxb.UnrecognizedContentError, CreateFromDocument, xmls.encode("utf-8")
        )

    def testExplicit(self):
        xmls = (
            '<ns:entry xmlns:ns="%s"><from>one</from><ns:to>single</ns:to></ns:entry>'
            % (Namespace.uri(),)
        )
