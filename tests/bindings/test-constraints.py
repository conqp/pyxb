...
TLA._CF_length = facets.CF_length(
    super_facet=datatypes.string._CF_length, value=datatypes.nonNegativeInteger(3)
)
...
Password._CF_minLength = facets.CF_minLength(
    super_facet=datatypes.string._CF_minLength, value=datatypes.nonNegativeInteger(8)
)
Password._CF_maxLength = facets.CF_maxLength(
    super_facet=datatypes.string._CF_maxLength,
    value=facets.CF_maxLength._ValueDatatype(15),
)
...
AFew._CF_minLength = facets.CF_minLength(
    super_facet=datatypes.string._CF_minLength, value=datatypes.nonNegativeInteger(2)
)
AFew._CF_maxLength = facets.CF_maxLength(
    super_facet=datatypes.string._CF_maxLength,
    value=facets.CF_maxLength._ValueDatatype(4),
)
...
Hand._CF_length = facets.CF_length(
    super_facet=datatypes.string._CF_length, value=datatypes.nonNegativeInteger(5)
)
...
Cardinals._CF_enumeration = facets.CF_enumeration(
    value_datatype=Cardinals,
    super_facet=datatypes.string._CF_enumeration,
    enum_prefix=None,
)
...
ExclusiveFloat._CF_minExclusive = facets.CF_minExclusive(
    super_facet=datatypes.float._CF_minExclusive,
    value_datatype=datatypes.float,
    value=datatypes.float(-5),
)
ExclusiveFloat._CF_maxExclusive = facets.CF_maxExclusive(
    super_facet=datatypes.float._CF_maxExclusive,
    value_datatype=datatypes.float,
    value=datatypes.float(7),
)
ExclusiveFloat._InitializeFacetMap(
    ExclusiveFloat._CF_minExclusive, ExclusiveFloat._CF_maxExclusive
)
...
FixedPoint._CF_totalDigits = facets.CF_totalDigits(
    super_facet=datatypes.decimal._CF_totalDigits,
    value=facets.CF_totalDigits._ValueDatatype(5),
)
FixedPoint._CF_fractionDigits = facets.CF_fractionDigits(
    super_facet=datatypes.decimal._CF_maxExclusive,
    value=facets.CF_fractionDigits._ValueDatatype(2),
)
FixedPoint._InitializeFacetMap(
    FixedPoint._CF_totalDigits, FixedPoint._CF_fractionDigits
)
...
ReplaceString._CF_whiteSpace = facets.CF_whiteSpace(
    value=facets._WhiteSpace_enum.replace, super_facet=datatypes.string._CF_whiteSpace
)
...
CollapseString._CF_whiteSpace = facets.CF_whiteSpace(
    value=facets._WhiteSpace_enum.collapse, super_facet=datatypes.string._CF_whiteSpace
)
...
class testDigits(unittest.TestCase):
    ...
    def assertAlmostEqual(self, v1, v2, *args, **kw):
        if isinstance(v1, datatypes.decimal) or isinstance(v2, datatypes.decimal):
...
class testWhitespace(unittest.TestCase):
    ...
    def testReplace(self):
        ...
        self.assertEqual(" test  ", self.__Replace.normalizeString("\ttest\n\r"))
        self.assertEqual(
            " test  too ", self.__Replace.normalizeString("\ttest\n\rtoo\n")
        )
        ...
        self.assertEqual(" test  ", self.__Replace.normalizeString(six.u("\ttest\n\r")))
        self.assertEqual(
            " test  too ", self.__Replace.normalizeString(six.u("\ttest\n\rtoo\n"))
        )
    ...
    def testReplaceString(self):
        ...
        self.assertEqual(
            " test  too ", ReplaceString(six.u("\ttest\n\rtoo\n"), _from_xml=True)
        )
    ...
    def testCollapse(self):
        ...
        self.assertEqual("test", self.__Collapse.normalizeString(six.u("\ttest\n\r")))
        self.assertEqual(
            "test too", self.__Collapse.normalizeString(six.u("\ttest\n\rtoo\n"))
        )
    ...
    def testCollapseString(self):
        ...
        self.assertEqual(
            "test too", CollapseString(six.u("\ttest\n\rtoo\n"), _from_xml=True)
        )
    ...
    def testApplyWhitespace(self):
        ...
        self.assertEqual(
            source,
            CollapseString(source, _apply_whitespace_facet=False, _from_xml=True),
        )
