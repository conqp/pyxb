...
class TestFAC(unittest.TestCase):
    ...
    def testKT2004(self):
        ...
        x = Sequence(
            NumericalConstraint(Symbol("a"), 0, 1), x, Choice(Symbol("a"), Symbol("d"))
        )
    ...
    def testK2010c(self):
        ...
        for word in Lbar:
            ...
            for c in word:
                mcfg.step(c)
            self.assertEqual(
                0, len(mcfg.acceptingConfigurations()), "accepting %s" % (word,)
            )
        cfg.reset()
        mcfg = MultiConfiguration(cfg)
        mcfg.step("a")
    ...
    def testTransitionLayers(self):
        a1 = All(
            NumericalConstraint(Symbol("a"), 0, 1),
            Symbol("b"),
            NumericalConstraint(Symbol("c"), 0, 1),
        )
        a2 = All(Symbol("d"), NumericalConstraint(Symbol("e"), 0, 1), Symbol("f"))
        tt = NumericalConstraint(
            Sequence(
                NumericalConstraint(a1, 0, 1),
                NumericalConstraint(a2, 0, 1),
                Symbol("l"),
            ),
            0,
            3,
    ...
    def testAllTree(self):
        ...
        ex = Sequence(
            NumericalConstraint(Symbol("f"), 0, 1),
            a1,
            NumericalConstraint(a2, 0, 1),
            Symbol("l"),
        )
